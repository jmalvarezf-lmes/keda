...
func (h *scaleHandler) GetScalersCache(scalableObject interface{}) (*cache.ScalersCache, error) {
	withTriggers, err := asDuckWithTriggers(scalableObject)
	if err != nil {
		return nil, err
	}
	...
	scalers := h.buildScalers(withTriggers, podTemplateSpec, containerName)

	c, err := cache.NewScalerCache(scalers, h.logger, h.recorder)
	if err != nil {
		return nil, err
	}
	h.scalerCaches[key] = c
...
func (h *scaleHandler) startPushScalers(ctx context.Context, withTriggers *kedav1alpha1.WithTriggers, scalableObject interface{}, scalingMutex sync.Locker) {
	logger := h.logger.WithValues("type", withTriggers.Kind, "namespace", withTriggers.Namespace, "name", withTriggers.Name)
	cache, err := h.GetScalersCache(scalableObject)
	if err != nil {
...
func (h *scaleHandler) buildScalers(withTriggers *kedav1alpha1.WithTriggers, podTemplateSpec *corev1.PodTemplateSpec, containerName string) []cache.ScalerBuilder {
	logger := h.logger.WithValues("type", withTriggers.Kind, "namespace", withTriggers.Namespace, "name", withTriggers.Name)
	var err error
	...
	for scalerIndex, t := range withTriggers.Spec.Triggers {
		...
		factory := func() (scalers.Scaler, error) {
			...
			return buildScaler(h.client, trigger.Type, config)
		}
